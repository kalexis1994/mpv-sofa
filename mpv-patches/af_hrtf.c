...
static void update_channel_hrir_ex(struct priv *p, int ch, int crossfade) {
    ...
    if (crossfade && pair->left[pair->active_idx].valid) {
        ...
        // Copy overlap state from active convolver to the new one.
        // convolver_set_hrir() zeros the overlap buffer, but zeroing the
        // overlap abruptly cuts off the ringing tail of the previous HRIR,
        // creating an audible click. Instead, we carry over the active
        // convolver's overlap so the tail rings out naturally through the
        // crossfade period. By the time the crossfade ends (1024 samples =
        // 4 blocks), all copied overlap has shifted out of the buffer and
        // the new convolver is fully self-consistent.
        memcpy(pair->left[next].overlap_buf, pair->left[active].overlap_buf,
               HRTF_FFT_N * sizeof(float));
        memcpy(pair->right[next].overlap_buf, pair->right[active].overlap_buf,
               HRTF_FFT_N * sizeof(float));
...
static void af_hrtf_process(struct mp_filter *f) {
    ...
    if (p->shared && p->sofa) {
        ...
        if (tone_active && p->test_tone_remaining <= 0) {
            ...
            if (p->test_tone_ch >= 0 && p->test_tone_ch < HRTF_MAX_CHANNELS)
